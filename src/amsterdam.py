...
class Amsterdam:
    ...
    def update_config(self):
        try:
            shutil.copytree(self.get_sys_data_dirs('config'),
                            os.path.join(self.basepath, 'config'))
            shutil.copytree(self.get_sys_data_dirs('docker'),
                            os.path.join(self.basepath, 'docker'))
        # FIXME
    ...
    def generate_template(self, options):
        template_path = os.path.join(self.get_sys_data_dirs('templates'),
        ...
        with open(template_path, 'r') as amsterdam_file:
            ...
            with open(os.path.join(self.basepath, 'docker-compose.yml'),
                      'w') as amsterdam_compose_file:
                if sys.version < '3':
                    amsterdam_compose_file.write(amsterdam_config)
                else:
                    amsterdam_compose_file.write(
                        bytes(amsterdam_config, 'UTF-8'))
        template_path = os.path.join(self.get_sys_data_dirs('templates'),
                                     'ethtool.conf.j2')
        ...
        with open(os.path.join(self.basepath, 'config', 'suricata',
                               'ethtool.conf'), 'w') as ethtool_compose_file:
            if sys.version < '3':
    ...
    def check_environment(self):
        try:
            ...
            pass
            raise AmsterdamException(
                "Name or data directory can't contain/finish with non ascii character")

        if " " in self.name:
            raise AmsterdamException(
                "Name or data directory can't contain/finish with space")
        ...
        if ':' in version:
            ...
            sys.stderr.write(
                "docker-compose version number '%s' is not handled by code\n" % (
                    version.rstrip()))
    ...
    def start(self, args):
        if not os.path.exists(
                os.path.join(self.basepath, 'docker-compose.yml')):
            sys.stderr.write(
                "'%s' directory does not exist or is empty, please run setup command\n" % (
                    self.basepath))
